...
# ============ আধুনিক UI থিম সেটআপ ============
# কালার স্কিম
COLOR_PRIMARY = "#2E7D32"       # গাঢ় সবুজ
COLOR_SECONDARY = "#4CAF50"     # মূল সবুজ
COLOR_ACCENT = "#81C784"        # হালকা সবুজ
COLOR_BG = "#F1F8E9"            # ব্যাকগ্রাউন্ড
COLOR_CARD = "#FFFFFF"          # কার্ড ব্যাকগ্রাউন্ড
COLOR_TEXT = "#1B5E20"          # টেক্সট
COLOR_TEXT_LIGHT = "#4A7C59"    # হালকা টেক্সট
COLOR_BUTTON_PRIMARY = "#2E7D32"
COLOR_BUTTON_SECONDARY = "#4CAF50"
COLOR_BUTTON_ACCENT = "#FF9800"
COLOR_BUTTON_DANGER = "#D32F2F"
COLOR_BUTTON_INFO = "#1976D2"

# ============ হেল্পার ফাংশন: কার্ড তৈরি ============
def create_card(parent, text="", bg_color=COLOR_CARD, padx=10, pady=10, ipadx=15, ipady=10):
    """একটি কার্ড-স্টাইল কন্টেইনার তৈরি করুন"""
    card = tk.Frame(parent, bg=bg_color, relief="raised", bd=1)
    card.pack(fill="x", padx=padx, pady=pady)
    return card

# ============ হেল্পার ফাংশন: স্টাইলিশ বাটন ============
def create_button(parent, text, command, bg_color=COLOR_BUTTON_PRIMARY, fg_color="white", 
                  font_size=11, width=15, padx=10, pady=5):
...
header_content = tk.Frame(header_frame, bg=COLOR_PRIMARY)
header_content.pack(pady=20)

# লোগো/আইকন
tk.Label(header_content, text="🌾", font=("Arial", 36), bg=COLOR_PRIMARY).pack()

# মূল শিরোনাম
tk.Label(header_content, text="স্মার্ট কৃষি পরিকল্পনা", 
        font=("Arial", 22, "bold"), bg=COLOR_PRIMARY, fg="white").pack(pady=5)

# সাবটাইটেল
tk.Label(header_content, text="আধুনিক কৃষি ব্যবস্থাপনা সমাধান", 
...
weather_result_label = tk.Label(app, textvariable=weather_result, wraplength=600, 
                                justify="left", fg=COLOR_TEXT, bg=COLOR_BG, 
...
result_label = tk.Label(result_card, textvariable=result, wraplength=600, 
                       justify="left", fg=COLOR_TEXT, bg=COLOR_CARD, 
                       font=("Arial", 10), padx=15, pady=10)
result_label.pack()

# ============ অনুসন্ধান কার্ড ============
search_card = tk.Frame(app, bg=COLOR_CARD, relief="raised", bd=1)
search_card.pack(fill="x", padx=20, pady=10)

# কার্ড হেডার
search_header = tk.Frame(search_card, bg=COLOR_ACCENT)
search_header.pack(fill="x")
tk.Label(search_header, text="🔍 ফসল অনুসন্ধান", 
        font=("Arial", 12, "bold"), bg=COLOR_ACCENT, fg="white", pady=8).pack()

search_content = tk.Frame(search_card, bg=COLOR_CARD)
search_content.pack(fill="x", padx=15, pady=12)

# অনুসন্ধান ইনপুট
tk.Label(search_content, text="ফসলের নাম লিখুন:", bg=COLOR_CARD, 
        font=("Arial", 10, "bold"), fg=COLOR_TEXT).pack(anchor="w", pady=(0, 5))
search_entry_frame = tk.Frame(search_content, bg=COLOR_CARD)
search_entry_frame.pack(fill="x", pady=5)

tk.Entry(search_entry_frame, textvariable=search_text, font=("Arial", 11), 
        width=25, relief="solid", bd=1).pack(side="left", padx=(0, 5), fill="x", expand=True)

# অনুসন্ধান বাটন
search_btn = create_button(search_entry_frame, "🔍 খুঁজুন", search_crop,
                           bg_color=COLOR_BUTTON_SECONDARY, font_size=10, width=10)
search_btn.pack(side="left")

# অন্যান্য ফসলের তথ্য বাটন
other_crops_btn = create_button(search_content, "➕ অন্যান্য ফসলের তথ্য", open_second_window,
                                bg_color=COLOR_BUTTON_ACCENT, font_size=10, width=20)
other_crops_btn.pack(pady=10)

# ============ দ্রুত অ্যাকশন সেকশন ============
quick_card = tk.Frame(app, bg=COLOR_CARD, relief="raised", bd=1)
quick_card.pack(fill="x", padx=20, pady=10)

# কার্ড হেডার
quick_header = tk.Frame(quick_card, bg=COLOR_ACCENT)
quick_header.pack(fill="x")
tk.Label(quick_header, text="⚡ দ্রুত অ্যাকশন", 
        font=("Arial", 12, "bold"), bg=COLOR_ACCENT, fg="white", pady=8).pack()

quick_grid = tk.Frame(quick_card, bg=COLOR_CARD)
quick_grid.pack(fill="x", padx=15, pady=12)
...
# কার্ড হেডার
features_header = tk.Frame(features_card, bg=COLOR_SECONDARY)
features_header.pack(fill="x")
tk.Label(features_header, text="✨ নতুন ফিচারসমূহ", 
        font=("Arial", 12, "bold"), bg=COLOR_SECONDARY, fg="white", pady=8).pack()
...
clear_frame.pack(fill="x", padx=20, pady=15)
...
clear_btn = create_button(clear_frame, "🧹 সবকিছু ক্লিয়ার করুন", clear_all,
                         bg_color=COLOR_BUTTON_DANGER, font_size=11, width=20)
clear_btn.pack(side="right")

# ============ ফুটার ============
footer_frame = tk.Frame(app, bg=COLOR_PRIMARY)
footer_frame.pack(fill="x", side="bottom", pady=(10, 0))
tk.Label(footer_frame, text="স্মার্ট কৃষি পরিকল্পনা © ২০২৪ | কৃষকের বন্ধু", 
        font=("Arial", 9), bg=COLOR_PRIMARY, fg="#E8F5E9", pady=8).pack()
